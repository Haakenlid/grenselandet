...
def fordeling():
    ...
    def finn_ledig_plass(participant):
        if len(participant['prioritised signups']) != 0:
            ...
            if my_signup.status == 2:  # Spilleder
                participant['sessions that overlap'].update(
                    session_dict['sessions that overlap'])
                finn_ledig_plass(participant)  # ser om det er ledig på neste i bunken.
            else:
                if session_dict['program session'] in participant['sessions that overlap']:
                    ...
                    if session_dict['full'] == False:  # ledig plass
                        ...
                        my_signup.ordering = len(session_dict['assigned signups'])
                        my_signup.status = 0
                        finn_ledig_plass(participant)  # ser om det er ledig på neste
    ...
    gamemasters = Signup.objects.filter(
        status__in=[Signup.GAME_HOST, Signup.GAME_MASTER]
    )
    ...
    while not_finished:
        ...
        for participant in list_of_participants:
            not_finished = not_finished or finn_ledig_plass(participant)
